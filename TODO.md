...
- [x] **T-5.17** Fixed training/serving skew: the ETL lowercases text but inference did not, so mixed-case input was silently encoded as unseen. Normalisation now runs inside the pipeline; the feature contract moved to `ml/features.py` so the pickled reference resolves in both containers
- [x] **T-X.15** `make etl` and `make verify` compare the deployed Glue script against the working tree. Running a job whose script is older than the local one produced a downstream failure that looked unrelated
- [ ] **T-6.10** Set `seed_sample_data = false` once a real feed exists (Q-01). Terraform must not own data in the raw zone beyond the committed fixture
